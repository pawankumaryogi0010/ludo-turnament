...
<body>
    ...
    <script>
        ...
        function loadStats(){
            fetch(BASE_PATH + '/api/admin_kyc.php?action=get_stats').then(handleApiResponse).then(d=>{
        ...
        function loadKycList(){
            ...
            fetch(`${BASE_PATH}/api/admin_kyc.php?action=list&status=${status}&offset=${offset}&limit=${state.limit}&search=${encodeURIComponent(state.search)}`).then(handleApiResponse).then(d=>{
                if(d.success){
