...
<body>
    ...
    <script>
        const SettingsApp = {
            settings: {}, csrfToken: '<?php echo $csrf_token; ?>', basePath: '<?php echo $basePath; ?>',
            init(){this.loadSettings()},
            loadSettings(){
                fetch(this.basePath+'/api/admin_settings.php?action=get_settings').then(r=>r.json()).then(d=>{if(d.success){this.settings=d.data.settings;this.render()}}).catch(()=>this.showToast('Error loading settings','error'))
            },
            render(){
