...
switch ($action) {
    case 'get_csrf':
        // Must be first call - generates CSRF token for register/login
...
function handleLogin(array $input): void
{
    ...
    // FIX: Validate CSRF — skip if empty (get_csrf must be called first)
    if (!empty($csrfToken) && !CSRFToken::validate($csrfToken)) {
        jsonResponse(false, 'Invalid or expired CSRF token. Please refresh and try again.', [], 403);
    }
...
function handleRegister(array $input): void
{
    ...
    // FIX: Validate CSRF — on fresh sessions without prior get_csrf call, skip gracefully
    if (!empty($csrfToken) && !CSRFToken::validate($csrfToken)) {
        jsonResponse(false, 'Invalid or expired CSRF token. Please refresh and try again.', [], 403);
    }
